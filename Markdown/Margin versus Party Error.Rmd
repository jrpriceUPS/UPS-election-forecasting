...
The lines of best fit plotted above are given by simple linear models with Republican Error, Democratic Error, and Marginal Bias as the response variables for the percent of undecided voters in any poll. The coefficients and R squared values for each  model can be seen in the table below:
...
It is worth noting how most of the values in this table are negative. Why? Undecided voters  or those who identify with neither of the primary candidates when polled often end up voting for one of the two major candidates. 
