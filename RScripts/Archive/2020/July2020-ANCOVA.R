# Slight modification of Kruschke code to do standard ANCOVA (for use in predicting bias from undecideds)
# Notes by Jake
...
genMCMC = function( datFrm , yName="y" , xNomName="xNom" , xMetName="xMet" ,
  ...
  model {
    for ( i in 1:Ntotal ) {
      y[i] ~ dt( mu[i] , 1/ySigma^2, nu )
      mu[i] <-a[xNom[i]] + aMet[xNom[i]]*xMet[i]
    }
    ySigma ~ dunif( residSD/100 , ySD*10 ) 
    nu ~  dexp(1/30.0) 
    for ( j in 1:NxNomLvl ) { a[j] ~ dnorm( 0.0 , 1/aSigma^2 ) 
                              aMet[j] ~ dnorm( 0 , 1/(2*ySD/xMetSD)^2 ) }
  ...
  parameters = c(  "a" , "aMet" , "aSigma" , "ySigma" , "nu" )
  adaptSteps = 500 
  burnInSteps = 1000 
