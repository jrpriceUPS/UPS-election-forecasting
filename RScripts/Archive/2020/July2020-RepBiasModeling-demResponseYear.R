# Second script used to model repBias with demBias, undecideds using JAGS
# demBias modeled with t-distribution, center determined by pollster + year (with interactions)
# repBias modeled with ANCOVA response to demBias and undecideds (demBias response allowed to depend on year)
#
# Notes by Jake 8/10/20

# Let us predict rebBias with demBias. 
# Version two to see if other parameters should be dependent upon year and/or pollster. 
...
genMCMC = function( datFrm , dembiasName = "demBias" , pollsterName = "pollster" , yearName = "year",
  ...
  model {
    ...
    repBias[poll] ~ dnorm((repResponsetoDemBias[year[poll]]*dembias[poll] + UndecidedResponse*undecided[poll]), 1/repBiasSpread^2)
    ...
    repResponsetoDemBias[year]  ~ dnorm( 0 , 1/(10)^2 )}
...
smryMCMC = function(  codaSamples , datFrm=NULL , dembiasName = "demBias" , pollsterName = "pollster" , yearName = "year",
  ...
  summaryInfo = rbind( summaryInfo , 
                       "repResponsetoDemBias" = summarizePost( mcmcMat[,paste("repResponsetoDemBias[",i,"]",sep="")] 
