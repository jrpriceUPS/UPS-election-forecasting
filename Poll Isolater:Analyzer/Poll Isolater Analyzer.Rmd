...
This file will analyze data from `r pollsterOfChoice1`, `r pollsterOfChoice2`, `r pollsterOfChoice3`, `r pollsterOfChoice4`, and `r pollsterOfChoice5` between the years of `r earlyYear` and `r lateYear`. This particular subset of data is shown in the table below:
...
#will come back later and make this table better.
...
#lattice::xyplot(onlyRecentData1$bias ~ onlyRecentData2$bias ,main="Scatter Plots by Pollster",xlab=pollsterOfChoice1, ylab=pollsterOfChoice2 )

#lattice::xyplot(total$electiondate ~ total$bias,main="Scatter Plots by Pollster",xlab="Election Date", ylab="Bias") 

#newdataframe=merge(onlyRecentData1,onlyRecentData2, onlyRecentData3, onlyRecentData4, onlyRecentData5, by.x="race", by.y="bias")
#simpledata = data.table()

#y <- read.csv(text="Race,Election Date, Bias 1, Bias 2, Bias 3, #Bias 4, Bias 5")
#y1 = data.frame(Race=NA, ElectionDate=NA, Bias1=NA, Bias2=NA, #Bias3=NA, Bias4=NA, Bias5=NA)[numeric(0), ]

#for (val in unique(total$race_id)){
  #rbind(y,list(total$race,total$electiondate,total$bias))
#}
  #x=c(total$race_id, total$pollster, total$polldate, total$bias)
 # rbind(simpledata, x)
#}
